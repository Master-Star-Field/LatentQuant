...
def compute_neighbor_indices(points: torch.Tensor, k: int = 16) -> torch.Tensor:
    ...
    # For large point clouds, use random sampling instead of full distance computation
    if num_points > 10000:
        # Random sampling approach - much faster
        neighbor_indices = torch.randint(0, num_points, (batch_size, num_points, k), device=device)
        return neighbor_indices
    
    # Compute pairwise distances for smaller point clouds
...
def compute_subsample_indices(points: torch.Tensor, ratio: float = 0.25) -> torch.Tensor:
    ...
    Returns:
        Subsample indices [B, M, 1] where M = int(N * ratio)
    """
    batch_size, num_points, _ = points.shape
    device = points.device
    ...
    indices = indices.unsqueeze(-1)  # [B, M, 1] - add third dimension
...
def compute_interpolation_indices(points: torch.Tensor, subsample_indices: torch.Tensor) -> torch.Tensor:
    ...
    Args:
        ...
        subsample_indices: Subsampled point indices [B, M, 1]
    ...
    # Remove the extra dimension from subsample_indices
    subsample_indices_flat = subsample_indices.squeeze(-1)  # [B, M]
    
    # Get subsampled points
    batch_indices = torch.arange(batch_size, device=device).unsqueeze(1)  # [B, 1]
    subsampled_points = points[batch_indices, subsample_indices_flat]  # [B, M, 3]
...
def preprocess_for_randlanet(points: torch.Tensor, colors: torch.Tensor, 
    ...
    for layer in range(num_layers):
        ...
        if layer < num_layers - 1:
            ...
            # Remove extra dimension from sub_idx for indexing
            sub_idx_flat = sub_idx.squeeze(-1)  # [B, M]
            current_points = current_points[batch_indices, sub_idx_flat]
            current_features = current_features[batch_indices, sub_idx_flat]
